...
namespace Microsoft.Boogie
{
    public class LinearHelper
    {
        ...
        public void AddDisjointnessAssumptions(Implementation impl)
        {
            ...
            foreach (Block header in loopHeaders)
            {
        ...
        private List<Expr> DisjointnessExprs(IEnumerable<Variable> availableVars)
        {
            ...
            foreach (string domainName in linearTypeChecker.linearDomains.Keys)
            {
                var permissionExprs = 
                    ...
                    .Union(domainNameToHoleVar.ContainsKey(domainName)
                        ? new List<Expr> {Expr.Ident(domainNameToHoleVar[domainName])}
                        : new List<Expr>());
                Expr expr = linearTypeChecker.DisjointnessExprForPermissions(domainName, permissionExprs);
